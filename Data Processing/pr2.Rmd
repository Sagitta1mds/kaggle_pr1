...
print(cat("Dataset has", dim(df_numeric)[1], "rows and", dim(df_numeric)[2], "columns", end="\n", file=""))
...
df_NA <- na.omit(df)
dim(df_NA)
# we might have to remove rows with NA values
# Replace NA values with median data
...
#col_numeric <- sapply(df_numeric, is.numeric)
#df_num <- df[,col_numeric]
...
training_size_stats2 <- data.frame(matrix(nrow = 10, ncol =3))
colnames(training_size_stats2) <- c("training_size", "train_rmse", "test_rmse")

for(i in 1:length(training_size)){
  resutls_0_1 <- RMSE(df_numeric, training_size[i])
  training_size_stats2[i,] <- c(training_size[i], resutls_0_1[1], resutls_0_1[2])
}
...
**A**: #maybe remove volumn 'Date', 'Year' and keep only 'Released_year'
...
# imdbrating and tomato rating quantiles

ggplot(df_num, aes(x=Budget^1.3, y=Gross)) +
  geom_point(shape=1) +
  geom_smooth()


### Power transformations should be before one-hot and binning
log_transform <- function(x){log10()}

num_col <- colnames(df_num)
num_col <- num_col[! num_col %in% 'Gross']
power_df <- data.frame(matrix(NA, nrow = nrow(df_num), ncol = 0))

for(j in 1:length(num_col)){
    log_new_col <- log10(df_num[,num_col[j]] + 0.01) #to avoid log10(0)
    power_df[, paste0('log_', num_col[j])] <- log_new_col
    sqr_new_col <- (df_num[,num_col[j]])^2
    power_df[, paste0('square_', num_col[j])] <- sqr_new_col
    cub_new_col <- (df_num[,num_col[j]])^3
...
lm_new_num <- lm(Gross~., data = combined_df)
summary(lm_new_num)
...
combined_df_wanted <- na.omit(combined_df_wanted)

#combined_df <- na.omit(combined_df)

temp <- combined_df_wanted[,sapply(combined_df_wanted, function(x) is.numeric(x))]

temp <- combined_df[,sapply(combined_df, function(x) is.numeric(x))]


lm_new_num <- lm(Gross~., data = temp)
summary(lm_new_num)
...
temp$Budget <- NULL
temp$Runtime <- NULL
temp$imdbRating <- NULL
temp$tomatoRating <- NULL
temp$Metascore <- NULL
temp$imdbVotes <- NULL
...
df_categ <- df_categ[, num_categ_cols]
df_categ <- na.omit(df_categ)

## model for categorical features
training_size <- seq(0.1, 1, length.out = 20)
training_size_stats_categ <- data.frame(matrix(nrow = 20, ncol =3))
colnames(training_size_stats_categ) <- c("training_size", "train_rmse_categorical", "test_rmse_categorical")

for(i in 1:length(training_size)){
  resutls_categ <- RMSE(df_categ, training_size[i])
...
training_size <- seq(0.1, 1, length.out = 20)
training_size_stats_all <- data.frame(matrix(nrow = 10, ncol =3))
colnames(training_size_stats_all) <- c("training_size", "train_rmse_allFeatures", "test_rmse_allFeatures")

for(i in 1:length(training_size)){
  resutls_allFeatures <- RMSE(df_categ, training_size[i])
  training_size_stats_all[i,] <- c(training_size[i], resutls_allFeatures[1], resutls_allFeatures[2])
}
```
